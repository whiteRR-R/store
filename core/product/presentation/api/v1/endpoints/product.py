...
@router.delete("/products/{product_id}/attributes/{attribute_id}", status_code=status.HTTP_200_OK)
@inject
async def remove_product_attribute(
    ...
    await use_case.execute(product_id=product_id, attribute_id=attribute_id)
