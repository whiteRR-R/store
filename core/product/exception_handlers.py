...
from domain.exceptions import InvalidValueException, NotFoundException, AlreadyExistException, InvalidOperationException
...
async def rollback_exception_handler(request: Request, exc: RollbackException):
    return JsonResponse(
        ...
        content={"detail": f"An error occurred, and the transaction has been rolled back. {str(exc)}"},
    )
...
async def invalid_operation_exception_handler(request: Request, exc: InvalidOperationException):
    return JsonResponse(
        status_code=status.HTTP_400_BAD_REQUEST,
        content={"detail": f"Invalid operations {str(exc)}"},
    )
